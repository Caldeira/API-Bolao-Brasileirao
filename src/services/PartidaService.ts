import { PartidaDTO } from "../@types/dto/brasileiraoClienteDTO";
...
import { IPartidaService } from "../@types/services/IPartidaService";
...
export class PartidaService implements IPartidaService {
  ...
  async gerarPartida(partidaResponse: PartidaDTO): Promise<Partida> {
    try {
      const partidaBD = await this.partidaRepository.findbySlug(
        partidaResponse.slug
      );
      if (partidaBD) {
      ...
      throw new Error(
        `Houve um erro ao gerar partidas. Motivo: ${error.message}`
      );
    }
  }

  private async atualizarPartida(
    partidaResponse: PartidaDTO,
    partida: Partida
  ): Promise<Partida> {
    ...
    if (partida.dataRealizacao && partidaResponse.data_realizacao_iso) {
      if (
        new Date(partidaResponse.data_realizacao_iso).getTime() !==
        partida.dataRealizacao.getTime()
      ) {
  ...
  private async partidasFactory(partidaResponse: PartidaDTO): Promise<Partida> {
    ...
    const mandante = await this.timeRepository.findByNome(
      partidaResponse.time_mandante.nome_popular
    );
    partida.mandante = mandante;

    const visitante = await this.timeRepository.findByNome(
      partidaResponse.time_visitante.nome_popular
    );
